...
if($msgarea=='' || $msgarea==$confirm_string || $msgarea=='Comments' || $msgarea=='email recipient address not found.' || $msgarea=='Please enter a valid email address.' || $msgarea=='Please check your message again.') {
	echo "Please check your message again.";
}

// else if($contact_name=='' || $contact_name=='Full Name' || $contact_name=='Please enter your name.') {
...
<form id="ajaxForm">
  <?php foreach($myrows as $myrow) { if($myrow->tname=='input') { ?>
    <input id="<?php echo $myrow->tid; ?>" class="form-input required" name="<?php echo $myrow->name; ?>" value="<?php echo $myrow->value; ?>" type="text" default="<?php echo $myrow->value; ?>" onfocus="$('#<?php echo $myrow->tid; ?>').css({'color':'#000'});"/>
  <?php } else { ?>
    <textarea id="<?php echo $myrow->tid; ?>" class="form-input" default="Comments" name="<?php echo $myrow->name; ?>" onfocus="$('#<?php echo $myrow->tid; ?>').css({'color':'#000'});"><?php echo $myrow->value; ?></textarea>
  <?php } } ?>
    <input id="check" name="check" type="checkbox" />
  <div class="submit learn-more-cont">
    <input name="action" type="hidden" value="ajaxcf" />
    <span id="submit_button" class="learn-more" onClick="check_fields();">submit</span>
  </div>
	<!-- <div id="request-invalid" class="invalid_holder"></div>
    <div id="request-success">
      <div id="success-msg">THANK YOU, YOUR REQUEST HAS BEEN SENT
        <span id="success-confirm-request"> </span> 
      </div>
    </div> -->
</form>
<?php
